...
class File
{
   ...
   public:
      File(const std::string& filename, Mode mode);
                                 // create
      File(const std::string& filename, Mode mode, int permissions);
      ~File();
      ...
      size_type Size();
      void Seek(size_type Offset, Whence whence = Whence::SET);
